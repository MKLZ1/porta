class Linea {
  ...
  setConfig(posXorigen,posYorigen,angulo,mitadTam){
        let dx = Math.cos(toRadians(angulo))*mitadTam;
        let dy = -Math.sin(toRadians(angulo))*mitadTam;
        this.p2.setX(posXorigen + dx);
        this.p2.setY(posYorigen + dy);
        this.p1.setX(posXorigen + (dx*-1));
        this.p1.setY(posYorigen + (dy*-1));
  ...
  setLineaCP(x1=0,y1=0,punto2){
    this.setLineC(x1,y1,punto2.getX(),punto2.getY());
  }
