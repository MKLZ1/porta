class Transformacion1KR extends Transformacion {
  constructor(kr, tiempoDescanso, condiciones) {
    ...
    this.addEstadoMoviendoseOBL(
      getAnimacion("KR_transformacion1_moviendose"),
      condiciones.movimientoActivo,
      3
    );
    this.addEstado(new EstadoAcelerar(kr));

    this.addEstadoMuerte(getAnimacion("KR_T1_muerte"));
...
class EstadoAcelerar extends EstadoJugador {
  ...
  constructor(kr) {
    ...
    this.desbloquearCuando(
      () => this.criatura.getBacteria() === this.criatura.objetivo.getBacteria()
    );
    this.setPermitirTransicion(true);
  }
