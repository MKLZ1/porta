class DatosTiled {
  ...
  constructor(objetoTiled, funcionSalida) {
    ...
    this.reportarSeccionCargada = () => {
      ...
      if (this.numSeccionesCargadas == 2) {
        //Cuando las capas y hojas de sprites estan cargadas
        this.cargarImagenMapa();
      } else if (this.numSeccionesCargadas == 3) {
        //imagenMapa Cargada
  ...
  cargarImagenMapa() {
    ...
    let grafico = cvImagenMapa.getContext("2d", { willReadFrequently: true });
    for (let capaActual of this.capasSprites) {
      if (!capaActual.esDibujoPlano()) continue;
      this.dibujarImagenCapa(capaActual, grafico);
    }
    this.imagenMapa = imageData_to_image(
      grafico.getImageData(0, 0, this.anchoMapa, this.altoMapa)
    );
  ...
  cargarObjetosDibujo() {
    for (let cs of this.capasSprites) {
      ...
      for (let capa of this.objetoTiled.layers) {
        if (capa.type !== "objectgroup" || capa.name !== cs.getNombre())
          continue;
